...
func _init(root:StarBotChatNodeRoot) -> void:
	...
	variable_type_more=[]
	variable_name_view=[]
	input_port_array=["Bool","String","Float"]
	output_port_array=[]
...
func process_input(id:String,input_port_data:Array,output_port_data:Array):
	#第0个端口的输入数据，获取方式以此类推
	input_port_data[0]
	if input_port_data[0] is bool  and input_port_data[1] is String and input_port_data[2] is float:
		#将true发送到第0个输出端口，并传入ID
		if input_port_data[0]:
			var data=get_autoload().write_data(input_port_data[1],input_port_data[2])
		return true
	return false
