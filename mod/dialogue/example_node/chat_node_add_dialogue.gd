...
func _init(root:StarBotChatNodeRoot) -> void:
	...
	input_port_name=["是否操作","用户ID","剧本名","剧本数据"]
	#输出节点类型数组
...
func process_input(id:String,input_port_data:Array,output_port_data:Array):
	if input_port_data[0] is bool and input_port_data[1] is String and input_port_data[2] is String and input_port_data[3] is String:
		...
		if input_port_data[0]:
			output_port_data[0]=ModLoader.get_autoload("dialogue/dialogue").add_dialogue(input_port_data[1],input_port_data[2],input_port_data[3])
		else:
